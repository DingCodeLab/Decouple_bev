...
class SelectPointSamples:
    """Filter background points near the bounding box.
    Args:
        bbox_enlarge_range (tuple[float], float): Bbox enlarge range.
    """

    def __init__(self,fg_weigth=2.0,point_sample_mask_thresold=0.0):
        self.fg_weigth = fg_weigth
    ...
    def __call__(self, data):
        ...
        valid_masks = foreground_masks.max(1)
        
        p = torch.ones(valid_masks.shape[0])
        p[valid_masks] = self.fg_weigth
        
        sample_ratio = np.random.random()
        while sample_ratio < 0.3:
            sample_ratio = np.random.random()
        
        indices = torch.multinomial(p, int(len(p)*sample_ratio), replacement=False)
